#include "../../../../include/main/cpp/gui/visualiser.h"
...
void Visualiser::init_ui()
{
    ...
    QFile unknown_img(":/resources/img/unknown.svg");
    ...
    if(row == 0 && column == 2)
    {
        ...
        if(ui->infoTable->item(row, column)->text() == "Running")
        {
            if(!playing_recording)
            {
                ...
                if(cpu >= 0)
                {
                    QFile img(":/resources/img/executing.svg");
                    diagram = new QGraphicsSvgItem(img.fileName());
                    diagram_cpu_label->setText("CPU: " + QString::number(cpu));
                }
                else
                {
                    QFile img(":/resources/img/ready.svg");
                    diagram = new QGraphicsSvgItem(img.fileName());
                }
            }
            else
            {
                QFile img(":/resources/img/running.svg");
                diagram = new QGraphicsSvgItem(img.fileName());
            }
        }
        ...
        {
            QFile img(":/resources/img/interruptible.svg");
            diagram = new QGraphicsSvgItem(img.fileName());
        }
        else if(ui->infoTable->item(row, column)->text() == "Disk Sleep")
        {
            QFile img(":/resources/img/uninterruptible.svg");
            diagram = new QGraphicsSvgItem(img.fileName());
        }
        else if(ui->infoTable->item(row, column)->text() == "Zombie")
        {
            QFile img(":/resources/img/zombie.svg");
            diagram = new QGraphicsSvgItem(img.fileName());
        }
        else if(ui->infoTable->item(row, column)->text() == "Stopped")
        {
            ui->stopButton->setText("Resume");
            QFile img(":/resources/img/stopped.svg");
            diagram = new QGraphicsSvgItem(img.fileName());
        }
        else
        {
            QFile img(":/resources/img/unknown.svg");
            diagram = new QGraphicsSvgItem(img.fileName());
        }
